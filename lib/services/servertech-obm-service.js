...
di.annotate(serverTechObmServiceFactory,
    new di.Inject('Q', 'OBM.base', '_', 'Services.Encryption')
);

function serverTechObmServiceFactory(Q, BaseObmService, _, encryption) {
    ...
    ServerTechObmService.prototype._runInternal = function (command, file) {
        return this.run({
            ...
            args: [
                '-v2c',
                '-c', encryption.decrypt(this.options.config.community),

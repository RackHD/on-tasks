...
function panduitObmServiceFactory(
    ...
    PanduitObmService.prototype.powerControl = function(control) {
        ...
        return Promise.map(self.options.config.pduOutlets, function(pduOutlet){
            ...
            assert.string(pduOutlet.host || self.options.config.host, "host should be defined as a string");
            assert.string(pduOutlet.community || self.options.config.community, "community should be defined as a string");
            //assert.string(pduOutlet.cyclePassword || self.options.config.cyclePassword, "cyclePassword should be defined as a string");
            ...
            var cyclePwd = pduOutlet.cyclePassword ||
                                ...
                                ('A' + ((outletNum.length === 1)? '0' + outletNum : outletNum ));

            return self._runInternal([self.mibCyclePwdCommand + '.' + pduNum + '.' + outletNum, 's', cyclePwd], host, community)
                .then(function () {
                    return self._runInternal([self.mibPowerCommand + '.' + pduNum + '.' + outletNum, 'i', controlCmd[control]], host, community);
                });
        });
    };
    ...
    PanduitObmService.prototype.powerStatus = function() {
        ...
        return Promise.map(self.options.config.pduOutlets, function(pduOutlet){
            ...
            return self._runInternal([self.mibStatusCommand + '.' + pduNum + '.' + outletNum], host, community, 'snmpwalk')
    ...
    PanduitObmService.prototype._runInternal = function (command, host, community, file) {
        return this.run({

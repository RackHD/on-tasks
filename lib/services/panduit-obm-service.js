...
di.annotate(panduitObmServiceFactory,
    new di.Inject('Promise', 'OBM.base', '_', 'Assert')
);

function panduitObmServiceFactory(Promise, BaseObmService, _, assert) {
    ...
    PanduitObmService.prototype.powerControl = function(control) {
        ...
        return Promise.map(self.options.config.mappings, function(mapping){

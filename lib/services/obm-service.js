...
di.annotate(obmServiceFactory, new di.Provide('Services.OBM'));
    ...
    ObmService.prototype.retryObmCommand = function (options) {
        ...
        function obmLoop(times, delay) {
            ...
            self.runObmCommand(nodeId, obmCommand)
                .then(function (out) {
                    //logger.debug("runObmCommand("+nodeId+","+obmCommand+") invoked, resulting resolved promise is "+out);
                ...
                .catch(function (error) {
                    //logger.debug("runObmCommand("+nodeId+","+obmCommand+") invoked, resulting rejected promise "+ error);

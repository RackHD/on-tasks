...
function obmServiceFactory(
    ...
    /**
     * Check whether the specified node doesn't support obmSettings
     *
     * @memberOf ObmService
     * @function
     *
     ...
     */
    ObmService.checkInvalidService = function(node, obmSettings) {
        var type = node.type;
        var reqService;
        var invalidService = '';
        ...
        if (type !== undefined) {
            _.forEach(obmSettings, function (setting) {
                reqService = setting.service;
                if (Constants.ObmSettings[type].indexOf(reqService) === -1) {
                    invalidService = reqService;
                    return false;
                }
            });
        }

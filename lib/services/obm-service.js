...
function obmServiceFactory(Q, _, injector, assert, waterline, Logger, config) {
    ...
    ObmService.prototype.retryObmCommand = function (options) {
        ...
        function obmLoop(times, delay) {
            ...
            self.runObmCommand(nodeId, obmCommand)
                .then(function (out) {
                    //logger.debug("runObmCommand("+nodeId+","+obmCommand+") invoked
                    //resulting resolved promise is "+out);
                ...
                .catch(function (error) {
                    logger.debug("runObmCommand("+nodeId+","+obmCommand+
                        ") invoked, resulting rejected promise "+ error);
                    logger.warning("error stacktrace"+ error.stack);
     ...
     * the power status.
     */
    ObmService.prototype.wrapWithStatusCheck = function (nodeId, obmCommand) {
        var self = this;
        logger.silly("Invoking wrapWithStatusCheck "+nodeId+" "+obmCommand);
        // Decide whether we have to run the command or not
        return self.powerStatus(nodeId)
            .then(function (on) {
                            ...
                            if (obmCommand === 'powerOn') {
                                ...
                                return Q.delay(500)
                                    .then(function () {
                                        ...
                                        return self.powerStatus(nodeId, false);
                                    });
                            } else {
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.powerOn = function (nodeId) {
        assert.string(nodeId, "Node identifier string");
        return this.wrapWithStatusCheck(nodeId, 'powerOn');
    };
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.powerOff = function (nodeId) {
        assert.string(nodeId, "Node identifier string");
        return this.wrapWithStatusCheck(nodeId, 'powerOff');
    };
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.softPowerOff = function (nodeId) {
        assert.string(nodeId, "Node identifier string");
        return this.wrapWithStatusCheck(nodeId, 'soft');
    };
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.reboot = function (nodeId) {
        var self = this;
        assert.string(nodeId, "Node identifier string");
        return self.powerOff(nodeId)
            .then(function () {
                return self.powerOn(nodeId);
            });
    };
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.reset = function (nodeId) {
        assert.string(nodeId, "Node identifier string");
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.softReset = function (nodeId) {
        var self = this;
        assert.string(nodeId, "Node identifier string");
        return self.softPowerOff(nodeId)
            .then(function () {
                return self.powerOn(nodeId);
            });
    };
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.powerStatus = function (nodeId, expected) {
        assert.string(nodeId, "Node identifier string");
        return this.retryObmCommand({
            nodeId: nodeId,
     ...
     * @returns {Q.Promise}
     */
    ObmService.prototype.setBootPxe = function (nodeId) {
        assert.string(nodeId, "Node identifier string");

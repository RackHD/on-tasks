...
function racadmSetBIOSJobFactory(
    ...
    function RacadmSetBiosJob(options, context, taskId) {
        ...
        this.cifsInfo = {user: this.userName, password: this.passWord , filePath: this.filePath};
    ...
    RacadmSetBiosJob.prototype._run = function() {
        ...
        waterline.nodes.findByIdentifier(self.nodeId)
            ...
            .then(function(results){
                assert.equal(results.jobStatus.status,"Completed",results.jobStatus.status);
                self._done();
            })
            .catch(function(err) {
                logger.error('The Error Is Found Here!', {error: err});
                self._done(err);
            });

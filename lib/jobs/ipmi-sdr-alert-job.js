...
function ipmiSdrPollerAlertJobFactory(
    ...
    IpmiSdrPollerAlertJob.prototype._determineAlert = function _determineAlert(data) {
        ...
        .then(function() {
            ...
            return waterline.workitems.find({ id: data.workItemId });
        })
        .then(function(workitem) {
            var conf = workitem.config;
            conf.inCondition = conf.inCondition || {};
            var alerts = _.transform(data.sdr, function(results, val) {
                var alertObj;
                if (val.Status !== 'ok' && !conf.inCondition[val['Sensor Id']]) {
        ...
        .catch(function(err) {
            logger.error("failed to update workitem", {error:err});
        });
    };

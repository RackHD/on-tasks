...
function ipmiJobFactory(BaseJob, ipmitool, parser, Logger, util, assert, Q, _, encryption) {
    ...
    IpmiJob.prototype.collectIpmiSel = function(data, count) {
        return ipmitool.selList(data.host, data.user, encryption.decrypt(data.password), count)
        .then(function (sel) {
    ...
    IpmiJob.prototype.collectIpmiSdr = function(machine) {
        ...
        return ipmitool.sensorDataRepository(host, user, encryption.decrypt(password))
        .then(function (sdr) {

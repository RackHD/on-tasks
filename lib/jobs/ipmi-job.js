...
function ipmiJobFactory(BaseJob, ipmitool, parser, Logger, util, assert, Q, _) {
    ...
    IpmiJob.prototype._run = function run() {
        ...
        this._subscribeRunIpmiCommand(this.routingKey, 'selInformation', this.ipmiSelInformationCallback);
        this._subscribeRunIpmiCommand(this.routingKey, 'selList', this.ipmiSelListCallback);
     ...
     * @memberOf IpmiJob
     */
    IpmiJob.prototype.ipmiSelListCallback = function(data) {
        var self = this;
        if (!data.host || !data.user || !data.password) {
            return;
        }
        if (self.concurrentRequests(data.host, 'selList')) {
            return;
        }
        self.addConcurrentRequest(data.host, 'selList');
        var defaultSelCount = 25;
        return self.collectIpmiSelList(data, defaultSelCount)
        .then(function(result) {
            data.selList = result;
            return self._publishIpmiCommandResult(self.routingKey, 'selList', data);
        })
        .catch(function (err) {
        ...
        .finally(function() {
            self.removeConcurrentRequest(data.host, 'selList');
        });
    };
     ...
     * @param count
     */
    IpmiJob.prototype.collectIpmiSelList = function(data, count) {
        return ipmitool.selList(data.host, data.user, data.password, count)
        .then(function (sel) {
            return parser.parseSelListData(sel);
        });
    };

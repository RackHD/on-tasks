...
function ipmiJobFactory(BaseJob, ipmitool, parser, Logger, util, assert, Q, _) {
    ...
    function IpmiJob(options, context, taskId) {
        ...
        this.collectIpmiSel = collectIpmiSel;
    ...
    IpmiJob.prototype._run = function run() {
        ...
        self._subscribeRunIpmiCommand(self.routingKey, 'sel', function(data) {
            ...
            return self.collectIpmiSel(data, ipmitool, parser)
            .then(function(result) {
                debugger;
                data.sel = result;
 ...
 * @param parser
 */
function collectIpmiSel(data, ipmitool, parser) {
    var host = data.host,
        user = data.user,
        password = data.password;

    return ipmitool.selList(host, user, password)
    .then(function (sel) {
...
function extractSdrData(sdrData, parser, _) {
    ...
    var thresholdResults = _.transform(thresholdData, function(result, v, k) {
        ...
        // Normally 'ok', otherwise alert-worthy
        result[k] = { value : v.Status, type: 'threshold' };
    });

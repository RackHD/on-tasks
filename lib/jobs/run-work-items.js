...
function runWorkItemsJobFactory(
    ...
    RunWorkItemsJob.prototype._executeSnmpPoller = function _executeSnmpPoller(workItem) {
        ...
        var snmpConfigFound = Promise.resolve();
        if (workItem.node) {
            snmpConfigFound = waterline.nodes.findOne(workItem.node).then(function (node) {
                if (node) {
                    //var snmpSetting = _.find(node.snmpSettings, { service: 'ipmi-obm-service' });
                    if (node.snmpSetting) {
                        return snmpSetting.config;
                    }
                }
            });
        }
        ...
        return snmpConfigFound.then(function (snmpSettings) {
            return Promise.resolve(
                _.assign(
                    {},
                    snmpSettings,

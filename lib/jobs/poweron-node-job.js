...
function poweronJobFactory(Logger, Q, assert, obmService) {
    ...
    PowerOnJob.prototype.run = function run() {
        logger.info("Running PowerOn Job for target "+this.options.target);
        return obmService.powerOn(this.options.target);
    };
    ...
    PowerOnJob.prototype.cancel = function cancel() {
        return Q.reject(new Error("Cancel for poweron not implemented"));
    };

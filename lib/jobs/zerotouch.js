...
function zerotouchJobFactory(BaseJob, templateService, Logger, assert, util, _) {
    ...
    ZerotouchJob.prototype._run = function() {
        var self = this;
        logger.info("Running zerotouch job.", {
            id: self.nodeId
        });

        return templateService.get(self.options.startupConfig)
        .then(function(startupConfig) {
            if (!startupConfig) {
                self._done(new Error("Startup config does not exist in templates"));
                return 'failed';
            }
            return templateService.get(self.options.bootConfig);
        })

...
di.annotate(obmControlJobFactory, new di.Inject('Logger', 'Q', 'Assert', 'Service.OBM', '_'));
function obmControlJobFactory(Logger, Q, assert, obmService, _) {
    ...
    ObmControlJob.prototype.run = function run() {
        logger.info("Running reboot job for target " + this.options.target);
     ...
     * @param {Object} [options]
     */
    ObmControlJob.create = function(options) {
        return new ObmControlJob(options);
    };

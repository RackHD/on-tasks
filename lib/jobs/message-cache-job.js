...
function pollerMessageCacheJobFactory(
    ...
    PollerMessageCacheJob.prototype._run = function run() {
        ...
        _.forEach([
            ...
            self._subscribeUcsCommandResult(
                self.routingKey,
                command,
                self.createSetUcsCommandResultCallback(command)
            );
        });
     ...
     * @memberOf PollerMessageCacheJob
     */
    PollerMessageCacheJob.prototype.createSetUcsCommandResultCallback = function(command) {
        var self = this;
        return function(data) {
            self.cacheSet(data.workItemId || 'unknown.ucs.' + command,
                _.omit(data, 'workItemId'));
        };
    };

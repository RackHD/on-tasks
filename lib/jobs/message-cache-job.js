...
function pollerMessageCacheJobFactory(
    ...
    PollerMessageCacheJob.prototype._run = function run() {
        ...
        self._subscribeMetricResult(self.routingKey, '*', self.createSetMetricResult);
    ...
    PollerMessageCacheJob.prototype.createSetIpmiCommandResultCallback = function(command) {
        return function(data) {
            this.cacheSet(data.workItemId || 'unknown.ipmi.' + command, _.omit(data, 'workItemId'));
        };
    };
     ...
     * @memberOf PollerMessageCacheJob
     */
    PollerMessageCacheJob.prototype.createSetMetricResult = function(data, metricName) {
        this.cacheSet(

...
module.exports = ipmiSelListPollerAlertJobFactory;
di.annotate(ipmiSelListPollerAlertJobFactory, new di.Provide('Job.Poller.Alert.Ipmi.SelList'));
di.annotate(ipmiSelListPollerAlertJobFactory, new di.Inject(
    'Job.Poller.Alert',
    'Logger',
    'Util',
    'Assert',
...
function ipmiSelListPollerAlertJobFactory(PollerAlertJob, Logger, util, assert, Promise, _) {
    var logger = Logger.initialize(ipmiSelListPollerAlertJobFactory);
     ...
     * @constructor
     */
    function IpmiSelListPollerAlertJob(options, context, taskId) {
        assert.object(context);
        assert.uuid(context.graphId);

        var subscriptionArgs = [context.graphId, 'selList'];

        IpmiSelListPollerAlertJob.super_.call(this, logger, options, context, taskId,
                '_subscribeIpmiCommandResult', subscriptionArgs);
    }
    util.inherits(IpmiSelListPollerAlertJob, PollerAlertJob);

    IpmiSelListPollerAlertJob.prototype._determineAlert = function _determineAlert(data) {
        ...
        var alertData = _.omit(data, 'selList');
    ...
    return IpmiSelListPollerAlertJob;
}

...
function snmpJobFactory(
    ...
    SnmpJob.prototype.concurrentRequests = function(host) {
        assert.string(host);
        if(!_.has(this.concurrent, host)){
            this.concurrent[host] = 0;
        }
        if(this.concurrent[host] >= this.maxConcurrent){
            return true;
        } else {
            return false;
        }
    ...
    SnmpJob.prototype.addConcurrentRequest = function(host) {
        assert.number(this.concurrent[host]);
        this.concurrent[host] += 1;
    };

    SnmpJob.prototype.removeConcurrentRequest = function(host) {
        assert.number(this.concurrent[host]);
        this.concurrent[host] -= 1;
    };
    ...
    SnmpJob.prototype._run = function() {
        ...
        self._subscribeRunSnmpCommand(self.routingKey, function(data) {
            if (self.concurrentRequests(data.host)) {
                return;
            }
            self.addConcurrentRequest(data.host);
            ...
            .finally(function() {
                self.removeConcurrentRequest(data.host);
            });
        });
    };

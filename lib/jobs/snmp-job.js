...
function snmpJobFactory(BaseJob, SnmpTool, encryption, Logger, util, assert) {
    ...
    SnmpJob.prototype._run = function run() {
        ...
        self._subscribeRunSnmpCommand(self.routingKey, function(data) {
            var snmptool = new SnmpTool(data.host, encryption.decrypt(data.community));

            return snmptool.collectHostSnmp(data.oids)

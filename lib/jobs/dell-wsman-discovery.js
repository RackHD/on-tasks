...
//        })
        ...
        self.options.ranges.forEach(function(entry){
        	...
        	subRanges.forEach(function(entry){
            	...
            	.then(function(result) {
            		return self.processDiscoveredSystems(result);
                })
        	});
        });
    }
    	...
    	.then(function(result){
    	    ...
    	    if(nodesToProcess === 0){
    	        if(--self.ipRangesToProcess === 0){
                    logger.info('DISCOVERY: All IP ranges processed.  Resolving relationships...');
                        ...
                        .then(function(node_){
                            ...
                            .then(function(){
                                if(self.doInventory.indexOf('true') !== -1){
                                    logger.info('Starting INVENTORY for node: ' + entry.summary.id);
     ...
     * @memberOf DellWsmanDiscoveryJob
     */
    DellWsmanDiscoveryJob.prototype.resolveRelationships = function () {
    	var self = this;

    	self.computeNodes.forEach(function(node){
            // Find enclosure for this node.
            logger.debug('Resolve relationship for node: ' + node.name);
            return waterline.catalogs.findLatestCatalogOfSource(node.id, 'deviceSummary')
            .then(function(result){
                if(result.data.serviceTag !== result.data.chassisServiceTag){
                    // Modular system
                    logger.debug(node.name + ' is Modular node.');
                    var enclosingNode = _.find(self.enclosureNodes, { 'name': result.data.chassisServiceTag } );
                    if(enclosingNode === undefined) {
                        logger.debug('Enclosing node for (' + node.name + ') not found in current discovery... Checking DB.');
                        return waterline.nodes.findByIdentifier(result.data.chassisServiceTag)
                        .then(function(previouslyDiscoveredNode){
                            if(previouslyDiscoveredNode === undefined) {
                                logger.error('Enclosing node for (' + node.name + ') not found.');
                            } else {
                                logger.debug('Enclosing node is: ' + previouslyDiscoveredNode.name);
                                self.setRelationships(node, previouslyDiscoveredNode);
                            }
                        });
                    } else {
                        logger.debug('Enclosing node is: ' + enclosingNode.name);
                        self.setRelationships(node, enclosingNode);
                    }
                } else {
                // monolithic, create enclosure for this node
                    logger.debug(node.name + ' is Monolithic node... Creating enclosure node.');
                    var newNode = {
                    ...
                    return waterline.nodes.create(newNode)
                    .then(function (node_) {
                        self.setRelationships(node, node_);
                    })
                }
            })
    	})

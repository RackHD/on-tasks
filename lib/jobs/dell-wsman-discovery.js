...
//        })
    ...
    DellWsmanDiscoveryJob.prototype.discover = function() {
        ...
        self.options.ranges.forEach(function(range){
            if(!range.credentials || !range.credentials.user || !range.credentials.password) {
                if(!self.options.credentials) {
                    logger.error('No credentials provided for range: (' + range.startIp + ' - ' + range.endIp + ')')
                } else {
...
//            var credential = {
//                "user": range.credentials.user || self.user,
//                "password": range.credentials.password || self.password
//            };
...
//        	    entry.credential = credential;
        ...
        // At this point, we've assigned a credential to every range, so we just pass null user/password
    ...
    DellWsmanDiscoveryJob.prototype.generateCredentialMapping = function() {
        ...
        //logger.debug('CREDENTIAL MAPPING: ' + JSON.stringify(ipToCredential, null, 4));
    ...
    DellWsmanDiscoveryJob.prototype.getIpv4Ranges = function(entry) {
		...
		if((current & 0xffffff00) == (last & 0xffffff00)){ // this is a valid range
			...
			return [{
	 	       ...
	 	       "credential": entry.credentials || {"credential":{"user": null, "password": null}}
			}];
		}
    ...
    DellWsmanDiscoveryJob.prototype.collectInventory = function (device, node) {
        if(self.doInventory.indexOf('true') !== -1  && device !== null){
            if(self.newChassisNodes.indexOf(node.id) !== -1 || self.newcomputeNodes.indexOf(node.id) !== -1){
                logger.info('Starting POST DISCOVERY config for node: ' + node_.name);
                workflowTool.runGraph(node_.id, 'Graph.Dell.Wsman.PostDiscovery', null)
            }
        }
...
//                                }
                    ...
                    if(!result){
                        ...
                        .then(function (node_) {
                            ...
                            return Promise.all([self.createWsmanObm(node_, device.ipAddress),
                                                self.createRedfishObm(node_, device.ipAddress, 1),
                            ...
                            .then(function(){
                                ...
                                .then(function(encNode){
                                    return self.createRedfishObm(encNode, device.ipAddress, 1)
                                    .then(function(){
                                        self.checkAllNodesProcessed();
                                        if(self.doInventory.indexOf('true') !== -1) {
                                            return self.collectInventory(device, node_)
    ...
    DellWsmanDiscoveryJob.prototype.createDefaultObm = function(node, ipAddr){
        ...
        var settings = {
            ...
            config: {"user": credential.user || self.dell.credentials.user,
    ...
    DellWsmanDiscoveryJob.prototype.createRedfishObm = function (node, ipAddr, retries) {
        ...
        var settings = {
            ...
            port: '',
            protocol: 'https',
            username: credential.user || this.dell.credentials.user,

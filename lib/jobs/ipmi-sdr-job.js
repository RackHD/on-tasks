...
function ipmiSdrJobFactory(BaseJob, ipmitool, parser, Logger, util, assert, Q, _) {
    ...
    IpmiSdrJob.prototype._run = function run() {
        ...
        self._subscribeRunIpmiSdrCommand(self.ipmiSdrRoutingKey, function(machine) {
            logger.silly("Polling IPMI SDR data.", {
                machine: machine
            });
            return self.collectIpmiSdr(machine, ipmitool, assert, parser, _)
            .then(function(data) {
                return self._publishIpmiSdrCommandResult(self.ipmiSdrRoutingKey, data);
            })
            .catch(function (err) {

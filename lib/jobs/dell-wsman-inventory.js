...
function DellWsmanInventoryJobFactory(BaseJob, Logger, Promise, util, waterline, encryption, configuration, _, WsmanTool, errors, uuid) {
    ...
    DellWsmanInventoryJob.prototype.initJob = function () {
        ...
        .then(function(obm) {
            ...
            var parse = urlParse(self.dellConfigs.services.inventory.host);
    ...
    DellWsmanInventoryJob.prototype.inventoryCallback = function inventoryCallback(data){
     ...
     DellWsmanInventoryJob.prototype.collectInventoryViaCallback = function() {
      	...
      	var rackHdCallback = self.dellConfigs.services.inventory.endpoints.callbackUri;
      	...
      	if(self.nodeType === 'compute'){
      		requestUri = self.dellConfigs.services.inventory.endpoints.serverCallback;
      	} else if(self.nodeType === 'enclosure'){
      		requestUri = self.dellConfigs.services.inventory.endpoints.chassisCallback;
      	}
     ...
     DellWsmanInventoryJob.prototype.collectInventory = function(wsman) {
        ...
        return wsman.clientRequest(self.dellConfigs.services.inventory.endpoints.hardware, 'POST', self.target)
        .then(function(response) {
        	return self.handleResponse(response, 'hardware');
        })
        .then(function() {
            return wsman.clientRequest(self.dellConfigs.services.inventory.endpoints.software, 'POST', self.target)
            .then(function(response) {
            	return self.handleResponse(response, 'software');
            })
        .then(function() {
            return wsman.clientRequest(self.dellConfigs.services.inventory.endpoints.nics, 'POST', self.target)
            .then(function(response) {

...
function baseJobFactory(eventsProtocol, taskProtocol, assert, util, Q, _) {
    ...
    BaseJob.prototype._publishRunSnmpCommand = function _publishRunSnmpCommand(machine) {
        assert.uuid(this.snmpRoutingKey);
        assert.object(machine);
        assert.isIp(machine.ip);
        assert.string(machine.communityString);

        taskProtocol.publishRunSnmpCommand(this.snmpRoutingKey);
    };

    BaseJob.prototype._subscribeRunSnmpCommand = function _subscribeRunSnmpCommand(callback) {
        var self = this;
        assert.func(callback);
        assert.uuid(self.snmpRoutingKey);
    ...
    BaseJob.prototype._publishRunIpmiSdrCommand = function _publishRunIpmiSdrCommand(machine) {
        ...
        taskProtocol.publishRunIpmiSdrCommand(this.ipmiSdrRoutingKey);
    };

    BaseJob.prototype._subscribeRunIpmiSdrCommand = function _subscribeRunIpmiSdrCommand(callback) {
        var self = this;
        assert.func(callback);
        assert.uuid(self.ipmiSdrRoutingKey);

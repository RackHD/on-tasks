...
function baseJobFactory(
    ...
    BaseJob.prototype.cleanup = function() {
        if (_.isFunction(this._cleanup)) {
            return Promise.resolve(this._cleanup());
        } else {
    ...
    BaseJob.prototype._subscribeSnmpCommandResult =
        ...
        var deferred = taskProtocol.subscribeSnmpCommandResult(uuid, function(data) {
            if (data && data.community) {
    ...
    BaseJob.prototype._subscribeMetricResult =
        ...
        var deferred = taskProtocol.subscribeMetricResult(uuid, metricName, function(data, metricName) {
            if (data && data.community) {
                data.community = encryption.encrypt(data.community);
            }
            if (data && data.password) {
    ...
    BaseJob.prototype._subscribeRunIpmiCommand =
        ...
        var deferred = taskProtocol.subscribeRunIpmiCommand(uuid, command, function(data) {
            if (data && data.password) {
                data.password = encryption.decrypt(data.password);
            }
            callback.call(self, data);

...
function installOsJobFactory(
    ...
    /**
     * @memberOf InstallOsJob
     * GraphProgress update after installation steps
     */
    InstallOsJob.prototype.updateGraphProgress = function(taskDescription, value) {
        var self = this;
        return waterline.graphobjects.findOne({instanceId: self.context.graphId})
        .then(function(graph){
            var taskProgress = {
                value: value,
                maximum: self.options.totalSteps,
                description: taskDescription
            };
            var progress = GraphProgress.create(graph, taskDescription);
            progress.updateTaskProgress(self.taskId, taskProgress, true);
            return eventsProtocol.publishProgressEvent(self.context.graphId,
                                                       progress.getProgressEventData());
        })
        .catch(function(error) {
            logger.error('Error updating graph progress event', {
                taskId: self.taskId,
    ...
    InstallOsJob.prototype._run = function() {
        ...
        return Promise.resolve().then(function() {
            ...
            self._subscribeRequestProfile(function() {
                ...
                self.updateGraphProgress(description, 1);
                return self.profile;
            });
            ...
            self._subscribeNodeNotification(self.nodeId, function(data) {
                ...
                self.updateGraphProgress(description, self.options.totalSteps);
                return self._done();
            });
        }).catch(function(err) {

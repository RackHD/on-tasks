...
function SnmpFactory(assert, parser, ChildProcess, Promise, _) {
    ...
    SnmpTool.prototype.runCommand = function(command, oid, maxRepititions) {
        if (!oid) {
        ...
        if (maxRepititions) {
            args.unshift('-Cr' + maxRepititions);
        }
        var childProcess = new ChildProcess(command, args);
        return childProcess.run({ maxBuffer: 2000 * 1024 });
    };
    ...
    SnmpTool.prototype.bulkget = function(oid, maxRepititions) {
        return this.runCommand('/usr/bin/snmpbulkget', oid, maxRepititions);
    };

    SnmpTool.prototype.bulkwalk = function(oid, maxRepititions) {
        return this.runCommand('/usr/bin/snmpbulkwalk', oid, maxRepititions);
    };
    ...
    SnmpTool.prototype.collectHostSnmp = function(oids, options) {
        ...
        return Promise.map(oids, function (oid) {
            return queryMethod(oid, options.maxRepititions)

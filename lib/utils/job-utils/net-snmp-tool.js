...
function SnmpFactory(assert, parser, ChildProcess, Promise, _) {
    ...
    SnmpTool.prototype.runCommand = function(command, oid, maxRepetitions) {
        ...
        var args = ['-Oqn', '-v2c', '-c', this.community, this.host].concat(oid.split(' '));
    ...
    function translate(helperArgs, oids) {

        var args = helperArgs.concat(oids.split(' '));
        var env = {
            "MIBS": "+ALL"
        };
        var childProcess = new ChildProcess('/usr/bin/snmptranslate', args, env);
        return childProcess.run();
    }

    SnmpTool.prototype.toSymbolic = function(oids) {
        return translate(['-OS'], oids);
    };

    SnmpTool.prototype.toNumeric = function(oids) {
        return translate(['-On'], oids);
    };

...
di.annotate(wsmanToolFactory, new di.Inject('Promise', '_', 'Logger', 'HttpTool'));

function wsmanToolFactory(Promise, _, Logger, HttpTool) {

    function WsmanTool(path, options) {
        this.settings = {};
        if(path) {
            var parse = urlParse(path);
            ...
            this.settings = {
                ...
                recvTimeoutMs: options.recvTimeoutMs || 30000
            };
        }
    }

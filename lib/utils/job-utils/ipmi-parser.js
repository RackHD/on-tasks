...
di.annotate(parseSensorsDataFactory, new di.Inject('_'));
function parseSensorsDataFactory(_) {
    /**
    ...
    function parseSelData(selData) {
        ...
        var selObj = {};
        var i;
        if (selData[0].indexOf('SEL Entry') >= 0)  {
            i = 0;
        } else if (selData[1].indexOf('SEL Entry') >= 0)  {
            i = 1;
        } else {
            throw new Error("SEL Data does not include expect SEL Entry values at start.");
        }
        for (i; i < selData.length; i+=2) {
            if (selData[i].indexOf('SEL Entry') >= 0) {
                var selEntry = selData[i].match(/\w*$/);
                if (selEntry) {
                    selObj[selEntry] = selData[i+1].split(',');
                }
            }
        }
        return selObj;

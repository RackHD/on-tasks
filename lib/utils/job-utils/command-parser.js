...
function commandParserFactory(Logger, Q) {
    ...
    CommandParser.prototype[ipmiFru] = function(data) {
        if (data.error) {
            return Q.resolve({ source: 'ipmi-fru', error: data.error });
        }
        try {
            var split = data.stdout.split('\n');
            _.remove(split, function(line) {
            ...
            return Q.resolve({ data: parsed, source: 'ipmi-fru', store: true });
        } catch (e) {
            return Q.resolve({ source: 'ipmi-fru', error: e });
        }
    };
    ...
    CommandParser.prototype.parseUnknownTasks = function(tasks) {
        var parsed = _.map(tasks, function(data) {
            var out;
            if (data.error) {
                return Q.resolve({ source: data.source, error: data.error });
            } else if (!data.stdout) {
                return Q.resolve({ source: data.source, error: new Error("No data") });
            }
            if (data.format === 'json') {
                try {
                    out = JSON.parse(data.stdout);
                } catch (e) {
                    return Q.resolve({ source: data.source, error: e });
                }
                return Q.resolve({
                    source: data.source,
                    data: out,
                ...
                xmlParser(data.stdout, function(err, out) {
                    if (err) {
                        return Q.resolve({ source: data.source, error: err });
                    } else {
                        return Q.resolve({
                            data: out,
                            source: data.source,

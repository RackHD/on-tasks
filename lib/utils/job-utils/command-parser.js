...
function commandParserFactory(Logger, Promise, _) {
    ...
    matchParsers.esxcliNetworkDriverVersion = {
        ...
        parsefunction: function (data) {
            if (data.error) {
                return Promise.resolve({ source: 'esxcli-network-driver-version', error: data.error });
            }
            try {
                var store = true;
                ...
                return Promise.resolve({ data: parsed, source: 'esxcli-network-driver-version', store: true });
            } catch (e) {

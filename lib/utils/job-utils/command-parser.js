...
function commandParserFactory(Logger, Promise, _) {
        ...
        try {
            ...
            var parsed = _.transform(lines, function(result, line) {
                if (line.startsWith('Handle')) {
                    ...
                    if (split.length === 1) {
                    ...
                    } else if (split[0].trim() === 'Installable Languages') {
                        /* Handle corner case where 'Installable Languages' looks like
                         * an object initially but should be an array:
                         *
                         ...
                         */
                        subKey = split[0].trim();
                        // BIOS Language Information won't be an array of objects,
                        // so don't bother checking _.isArray() like we do above.
                        result[key][subKey] = [];
                    } else {
                        subKey = split[0].trim();
                        if (_.isArray(result[key])) {
    ...
    CommandParser.prototype[lshw] = function(data) {
        ...
        try {
            var parsed = JSON.parse(data.stdout);
            return Promise.resolve({ data: parsed, source: 'lshw', store: true });
        } catch (e) {

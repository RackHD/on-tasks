...
function taskGraphFactory(
    ...
    TaskGraph.updateGraphProgress = function(progressData) {
        ...
        if (_.has(progressData, 'progress.percentage')) {
            progressData.progress.percentage =
                _alignInvalidPercent(progressData.progress.percentage);
        }
        if (_.has(progressData, 'taskProgress.progress.percentage')) {
            progressData.taskProgress.progress.percentage =
                _alignInvalidPercent(progressData.taskProgress.progress.percentage);
        }
        ...
        .tap(function(){
            ...
            if (_.has(progressData, 'taskProgress') &&
                ...
                return store.updateTaskProgress(progressData.taskProgress, progressData.graphId);
            }
        })
    ...
    function _alignInvalidPercent(percent) {
        var pattern = /(100|\d{1,2})(\.\d{0,})?\%/;
        var percentFormatInvalid = !percent || !percent.match(pattern);
        if (percentFormatInvalid) {

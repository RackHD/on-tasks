...
function taskGraphFactory(
     ...
     * @returns {Promise}
     */
    TaskGraph.updateGraphProgress = function(graphId, data) {
        ...
        data.progress = _validateProgressData(data.progress);

        if (_.has(data, 'taskProgress.progress')){
            data.taskProgress.progress = _validateProgressData(data.taskProgress.progress);
        }
        ...
        return Promise.resolve()
        .then(function(){
            //Find available graphName if it doesn't exist in data
        ...
        .then(function(graph){
            //Find available taskName if it doesn't exist in data
            if (!_.has(data, 'taskProgress') || data.taskProgress === {}) {
                return;
            }
            if (!_.has(data, 'taskProgress.taskId')){
                return;
            }
            ...
            if (_.has(graph, 'tasks') &&
                ...
                _.has(graph.tasks[data.taskProgress.taskId], 'friendlyName')) {
                data.taskProgress.taskName = graph.tasks[data.taskProgress.taskId].friendlyName;
            }
        })
        .then(function(){
            return taskMessenger.publishProgressEvent(data);
        });
    };

    function _validateProgressData(data) {

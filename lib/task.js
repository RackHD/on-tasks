...
function factory(Logger, assert, uuid, Q, _, injector) {
    ...
    Task.create = function create(optionOverrides) {
        return new Task(optionOverrides);
    };

    Task.createRegistryObject = function (definition) {
        return {
            create: function(optionOverrides, taskOverrides) {
                var _definition = definition;
                var options = _.defaults(_definition.options, optionOverrides);

 ...
 * @param logger
 */
function factory(taskProtocol, Logger, assert, uuid, Q, _, injector) {
    ...
    function Task(definition, taskOverrides) {
        ...
        this.failedStates = ['failed', 'timeout', 'canceled'];
    ...
    Task.prototype.run = function () {
        ...
        self.state = 'started';
        return self.job.run().then(function() {
            if (!self.outcome) {
    ...
    Task.prototype.cancel = function() {
        var self = this;
        return self.job.cancel().catch(function(error) {
    ...
    */
    ...
    Task.start = function start() {
        var self = this;
        return Q.all([
            taskProtocol.subscribeRun(self.instanceId, self.run),
            taskProtocol.subscribeCancel(self.instanceId, self.cancel)
        ]).then(function() {
    ...
    Task.create = function create(definition, taskOverrides) {
        var task = new Task(definition, taskOverrides);
        task.start();
        return task;
    };

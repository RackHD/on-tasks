...
function factory(
    ...
    function renderString(str, context, depth, maxDepth) {
        if (depth > maxDepth) {
        ...
        var rendered;
        _.some(str.split(/[\s\n]?\|+[\s\n]?/), function(item) {
            rendered = item.indexOf('.') > -1 ? getPath(context, item) : item;
            return rendered;
        });
        if (!rendered) {
        ...
        var partialRender = _.some(parse(rendered), function(item) {
            return item.tag === '_v';
        });
        ...
        if (partialRender) {
            return render(rendered, context, depth + 1);
        } else {
            return rendered;
        }
    }
    ...
    function render(source, context, depth) {
        ...
        return _.map(parsed, function(item) {
            ...
            if (item.tag === '_t') {
                ...
                return renderString(item.n, context, depth, 50);
            }
        }).join('');
    }
    ...
    function Task(definition, taskOverrides, context) {
        ...
        var renderContext = {
            server: configuration.getAll(),
            api: {
                server:
                    'http://' + configuration.get('server') + ':' + configuration.get('httpPort'),
                httpsServer:
                    'https://' + configuration.get('server') + ':' + configuration.get('httpsPort'),
            },
            task: self,
            options: self.definition.options
        };
        renderContext.api.base = renderContext.api.server + '/api/current';
        renderContext.api.baseHttps = renderContext.api.httpsServer + '/api/current';
        renderContext.api.files = renderContext.api.base + '/files';
        renderContext.api.filesHttps = renderContext.api.baseHttps + '/files';

        self.options = self.renderOptions(_.cloneDeep(self.definition.options), renderContext);
    ...
    Task.prototype.renderOptions = function(toRender, renderContext) {
        ...
        if (_.isEmpty(toRender)) {
            ...
            return render(toRender, renderContext);
        } else {
